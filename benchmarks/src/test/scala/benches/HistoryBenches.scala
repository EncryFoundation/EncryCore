...
class HistoryBenches {
  ...
  def appendBlocksToHistoryBench(benchStateHistory: HistoryBenchState, bh: Blackhole): Unit = {
    bh.consume {
      val history: HistoryImpl = generateHistory(benchStateHistory.settings, getRandomTempDir)
  ...
  def readHistoryFileBench(benchStateHistory: HistoryBenchState, bh: Blackhole): Unit = {
    bh.consume {
      val history: HistoryImpl = generateHistory(benchStateHistory.settings, benchStateHistory.tmpDir)
      history.closeStorage()
    }
  }
}
...
object HistoryBenches {
  ...
  class HistoryBenchState {
    ...
    val initialHistory: HistoryImpl = generateHistory(settings, tmpDir)

    val resultedHistory: (HistoryImpl, Option[Block], Vector[Block]) =
