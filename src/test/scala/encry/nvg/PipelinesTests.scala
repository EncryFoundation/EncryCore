...
object PipelinesTests extends InstanceFactory {

  val key: PrivateKey25519 = TestHelper.genKeys(1).head

  def generateValidForHistoryAndStateBlocks(blocksQty: Int): (History, UtxoState, List[Block]) = {
    (0 to blocksQty).foldLeft(generateDummyHistory(settings),
                              UtxoState.genesis(
