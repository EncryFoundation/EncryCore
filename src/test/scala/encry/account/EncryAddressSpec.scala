...
class EncryAddressSpec extends PropSpec with Matchers {
  ...
  val validP2SH: Address = Address @@ Base58Check.encode(Pay2ScriptHashAddress.TypePrefix +: PubKeyLockedContract(pubKey).contract.hash)
  ...
  val p2sh: Pay2ScriptHashAddress = Pay2ScriptHashAddress(validP2SH)
