...
//            val settingsNew                          = settings.copy(snapshotSettings = settings.snapshotSettings.copy(newSnapshotCreationHeight = 50))
...
////          createAvl("9gKDVmfsA6J4b78jDBx6JmS86Zph98NnjnUqTJBkW7zitQMReia", 5, 25)
////          val block1: Block = generateGenesisBlock(Height @@ 1)
////          val block2: Block = generateGenesisBlock(Height @@ settingsNew.snapshotSettings.newSnapshotCreationHeight)
////          val block3: Block =
////          generateGenesisBlock(Height @@ (settingsNew.snapshotSettings.newSnapshotCreationHeight + settings.levelDB.maxVersions))
////
////          val processor1: SnapshotProcessor =
////          snapshotProcessor.processNewSnapshot(UtxoState(avl1, Height @@ 0, settings.constants), block1)
////          val manifest1: SnapshotHolder.SnapshotManifest = processor1.bestPotentialManifest.get
...
////
////          val block4: Block =
////          generateGenesisBlock(Height @@ (settings.levelDB.maxVersions + settings.snapshotSettings.newSnapshotCreationHeight * 2))
////
////          val processor1                                         = snapshotProcessor.processNewSnapshot(UtxoState(avl1, Height @@ 0, settings.constants), block1)
////          val manifest1: Option[SnapshotHolder.SnapshotManifest] = processor1.bestPotentialManifest
////
...
////          Height @@ ((settings.levelDB.maxVersions - 1) + settings.snapshotSettings.newSnapshotCreationHeight * 2)
////          )
////
////          val processor1 = snapshotProcessor.processNewSnapshot(UtxoState(avl1, Height @@ 0, settings.constants), block1)
////
...
////        val snapshotProcessor: SnapshotProcessor = SnapshotProcessor.create(settings, tmpDir)
////        val avl1: AvlTree[StorageKey, StorageValue] =
////          createAvl("9gKDVmfsA6J4b78jDBx6JmS86Zph98NnjnUqTJBkW7zitQMReia", 0, 50000)
////        val fromFirst  = avl1.createSubtrees.map(list => ByteArrayWrapper(list.head.hash))
////        val fromSecond = avl1.getChunks(avl1.rootNode, 1).map(node => ByteArrayWrapper(node.head.hash))
////
////        assert(fromFirst.length == fromSecond.length, "not same length")
////        assert(fromFirst.forall(fromSecond.contains), "not all elem contains in second")
////
////        val a = avl1.createSubtrees.flatten.map(r => ByteArrayWrapper(r.hash))
////        val b = avl1.getChunks(avl1.rootNode, 1).flatten.map(r => ByteArrayWrapper(r.hash))
////
////        a.forall { elem =>
////          b.contains(elem)
////        } shouldBe true
  ...
  "Snapshot processor" should {
    ...
    "process new block function" should {
      "process block correctly if condition == 0" in {
        val sn = settings.copy(snapshotSettings = settings.snapshotSettings.copy(newSnapshotCreationHeight = 9),
                               levelDB = settings.levelDB.copy(maxVersions = 4))
        ...
        val processor3 =
          processor2.processNewBlock(newBlocks.last, historyNew1).right.get
      ...
      "update actual snapshot correctly" in {}
    }
  }
