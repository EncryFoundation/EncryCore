...
class EncryStateSpec extends PropSpec with Matchers with EncryGenerator {
  ...
  property("EncryState.genesisBoxes() output equality") {
    ...
    val g1: Seq[AssetBox] = EncrySupplyController.initialStateBoxes
    val g2: Seq[AssetBox] = EncrySupplyController.initialStateBoxes
    val g3: Seq[AssetBox] = EncrySupplyController.initialStateBoxes
