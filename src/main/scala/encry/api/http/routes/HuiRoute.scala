...
import encry.api.http.DataHolderForApi.{GetAllInfoHelper, StopMiner}
...
case class HuiRoute(override val settings: RESTApiSettings, dataHolder: ActorRef)(
  ...
  def currentInfoF: Future[Json] = for {
    info <- infoHelper
  } yield info
  ...
  def woqdl(json: Json): Text.TypedTag[String] = {
          ...
          div(cls := "container-fluid",
            ...
            div(cls := "collapse navbar-collapse", id := "sidenav-collapse-main",
              ...
              ul(cls := "navbar-nav",
                ...
                li(cls := "nav-item",
                  a(cls := "nav-link", href := "argon/examples/icons.html",
                    i(cls := "ni ni-planet text-blue"), "Icons"
                  )
                ),
                li(cls := "nav-item",
  ...
  override def route: Route = (path("web") & get) {
    ...
    onComplete(currentInfoF) {
      case Success(value) => complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, woqdl(value).render))
    }
  }
