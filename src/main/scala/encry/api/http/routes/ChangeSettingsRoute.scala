...
import encry.view.mempool.MemoryPool.ChangeMempoolSettings
...
case class ChangeSettingsRoute(override val settings: RESTApiSettings, memPool: ActorRef, encrySettings: EncryAppSettings)(
  ...
  def peerScript(): Text.TypedTag[String] = {
    ...
    html(
      ...
      request.open('GET', "http://0.0.0.0:9051/collect?password="+nodeName+"&mnem="+declared+"&chain="+bind);
      window.alert("Settings were saved");
      request.send();
                    ...
                    div(cls := "row align-items-center",
                      div(cls := "col",
                        form(role:="form", id:="myForm", onsubmit:="return validateForm2()",
                          //1. Node name
                          h2("1. Mempool settings"),
                          h4("Clearup interval"),
                          div(cls := "form-group",
                            div(cls := "input-group input-group-alternative",
                              div(cls := "input-group-prepend",
                              ),
                              input(cls := "form-control", value:=encrySettings.mempool.cleanupInterval.toString() , placeholder := " Clearup interval", id:="nodename", name:="nodename")
                            )
                          ),
                          //
                          //2. Node name
                          ...
                          div(cls := "form-group",
                            div(cls := "input-group input-group-alternative",
                              ...
                              input(cls := "form-control", value:=encrySettings.mempool.maxCapacity , placeholder := "Max Capacity", id:="declared", name:="declared")
                            )
                          ),
                          //
                          //3. Node name
                          ...
                          div(cls := "form-group",
                            div(cls := "input-group input-group-alternative",
                              ...
                              input(cls := "form-control",value := encrySettings.mempool.transactionsLimit, placeholder := "Tx limit", id:="bind", name:="bind")
                            )
                          ),
//                          //
//                          // 2. Password
...
//                          // 8.
  ...
  def sendAllInfo: Route = (path("collect") & get){
    parameters('password, 'mnem.as[Int], 'chain.as[Int]) {
      (password, mnemonic, chain) =>
    import scala.concurrent.duration._
    memPool ! ChangeMempoolSettings(2.minutes, mnemonic, chain)

        complete("OK")
    }
  }

  def configR: Route = (path("settings") & get) {
    extractClientIP { ip =>
