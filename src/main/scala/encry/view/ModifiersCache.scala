...
import encry.view.history.HistoryValidationError.{FatalValidationError, NonFatalValidationError}
...
object ModifiersCache extends StrictLogging {
  ...
  def findCandidateKey(history: EncryHistory): List[Key] = {
    ...
    def exhaustiveSearch: List[Key] = List(cache.find { case (k, v) =>
      v match {
        case _: Header if history.getBestHeaderIdOpt.exists(headerId => headerId sameElements v.parentId) => true
        case _ =>
      ...
      headersCollection.get(history.getBestHeaderHeight + 1) match {
        case Some(value) =>
          ...
          val res = value.map(cache.get(_)).collect {
            case Some(v: Header)
              if ((v.parentId sameElements history.getBestHeaderIdOpt.getOrElse(Array.emptyByteArray)) ||
