...
class WalletDBImpl(
  ...
  @tailrec
  private def computePredicate[BXT: ClassTag](acc: List[BXT], ids: List[ADKey], f: List[BXT] => Boolean): List[BXT] =
    ids.headOption match {
      case Some(boxId: ADKey) =>
        val newAcc: List[BXT] = getTypedBoxById[BXT](boxId).fold(acc)(_ :: acc)
        if (f(newAcc)) newAcc else computePredicate(newAcc, ids.drop(1), f)
  ...
  ): List[AssetBox] = computePredicate[AssetBox](
    List.empty[AssetBox],
  ...
  ): List[TokenIssuingBox] = computePredicate[TokenIssuingBox](
  ...
  ): List[DataBox] = computePredicate[DataBox](
    List.empty[DataBox],
  ...
  override def updateWallet(
    ...
    println(
      s"infoAboutWalletToInsert -> ${infoAboutWalletToInsert.map(l => Algos.encode(l._1) -> Algos.encode(l._2)).mkString(",")}"
    )
