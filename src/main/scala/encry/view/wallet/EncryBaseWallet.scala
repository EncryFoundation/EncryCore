...
import scorex.core.transaction.wallet._
...
case class WalletTransaction[TX <: EncryBaseTransaction](proposition: Proposition,
                                                         tx: TX,
                                                         blockId: Option[ModifierId],
                                                         createdAt: Long)

object WalletTransaction {
  def parse[TX <: EncryBaseTransaction](bytes: Array[Byte])
                                       (propDeserializer: Array[Byte] => Try[Proposition],
                                        txDeserializer: Array[Byte] => Try[TX]): Try[WalletTransaction[TX]] = Try {
    ...
    WalletTransaction[TX](propTry.get, txTry.get, blockIdOpt, createdAt)
  }

  def bytes[TX <: EncryBaseTransaction](wt: WalletTransaction[TX]): Array[Byte] = {
...
/**
  ...
  * @tparam TX
  */
trait EncryBaseWallet[TX <: EncryBaseTransaction, PMOD <: PersistentNodeViewModifier, W <: EncryBaseWallet[TX, PMOD, W]]
  extends EncryVault[PMOD, W] {
  self: W =>
  ...
  def historyTransactions: Seq[WalletTransaction[TX]]
