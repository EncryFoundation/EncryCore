    ...
    modifier match {
      case a : EncryBlockPayload => a.transactions.foldLeft(this) { case (w, tx) =>
        tx match {
          //TODO: not efficient
          case sp: PaymentTransaction =>
            if ((sp.senderProposition.bytes sameElements secret.publicKeyBytes) || sp.createBoxes.foldRight(false) { (a: (Address, Amount), b: Boolean) => a._1.getBytes() sameElements secret.publicKeyBytes }){
