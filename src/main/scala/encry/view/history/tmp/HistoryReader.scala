...
trait HistoryReader {
  ...
  protected[history] var isHeadersChainSyncedVar: Boolean = false
  ...
  final protected[history] def addHeaderToCacheIfNecessary(h: Header): Unit =
  ...
  final protected[history] def addBlockToCacheIfNecessary(b: Block): Unit =
