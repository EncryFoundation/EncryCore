...
class HistoryModifiersValidator(
  ...
  lazy val powScheme: EquihashPowScheme = EquihashPowScheme(
    settings.constants.n,
  ...
  private def headerValidator(h: Header, parent: Header): Either[ValidationError, Header] =
    for {
      ...
      _ <- Either.cond(
            isSemanticallyValid(h.parentId) != ModifierSemanticValidity.Invalid,
            (),
