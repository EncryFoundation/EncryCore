...
import encry.network.AuxiliaryHistoryHolder.{Append, AuxHistoryChanged, ReportModifierInvalid, ReportModifierValid}
...
class EncryNodeViewHolder[StateType <: EncryState[StateType]](auxHistoryHolder: ActorRef) extends Actor with StrictLogging {
  ...
  system.scheduler.schedule(10.seconds, 10.seconds, nodeViewSynchronizer, AuxHistoryChanged(nodeView.history))
