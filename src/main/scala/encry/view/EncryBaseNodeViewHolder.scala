...
import encry.EncryLocalInterface.{LocallyGeneratedModifier, LocallyGeneratedTransaction}
import encry.modifiers.EncryTransactionCarryingPersistentModifier
import encry.modifiers.mempool.EncryBaseTransaction
import encry.modifiers.state.TransactionValidator
import encry.view.mempool.{EncryBaseMemoryPool, EncryMempoolReader}
import encry.view.wallet.EncryVault
import scorex.core._
import scorex.core.consensus.History.ProgressInfo
import scorex.core.consensus.{History, HistoryReader, SyncInfo}
import scorex.core.network.{ConnectedPeer, NodeViewSynchronizer}
import scorex.core.network.NodeViewSynchronizer.{CompareViews, ModifiersFromRemote}
import scorex.core.serialization.Serializer
import scorex.core.transaction.{MemoryPool, MempoolReader, Transaction}
...
/**
  ...
  * @tparam PMOD
  */
trait EncryBaseNodeViewHolder[PMOD <: PersistentNodeViewModifier]
  extends Actor with ScorexLogging {

  import EncryBaseNodeViewHolder._
  ...
  type VL <: EncryVault[PMOD, VL]
  type MP <: EncryBaseMemoryPool[MP]
  ...
  private val subscribers = mutable.Map[EncryBaseNodeViewHolder.EventType.Value, Seq[ActorRef]]()
  ...
  protected def txModify(tx: TX): Unit = {
    ...
    val errorOpt: Option[Throwable] = minimalState() match {
      case txValidator: TransactionValidator =>
      ...
      case None =>
        memoryPool().put(tx) match {
          case Success(newPool) =>
            ...
            notifySubscribers(EventType.SuccessfulTransaction, SuccessfulTransaction[TX](tx))

          case Failure(e) =>
            notifySubscribers(EventType.FailedTransaction, FailedTransaction[TX](tx, e))
        }
  ...
  protected def updateNodeView(updatedHistory: Option[HIS] = None,
    ...
    if (updatedMempool.nonEmpty) {
      notifySubscribers[ChangedMempool[EncryMempoolReader]](EventType.MempoolChanged, ChangedMempool(newNodeView._4.getReader))
    }
    nodeView = newNodeView
  }

  protected def extractTransactions(mod: PMOD): Seq[TX] = mod match {
    case tcm: EncryTransactionCarryingPersistentModifier => tcm.transactions
    case _ => Seq()
  }
  ...
  protected def updateMemPool(progressInfo: History.ProgressInfo[PMOD], memPool: MP, state: MS): MP = {
    ...
    memPool.putWithoutCheck(rolledBackTxs).filter { tx =>
      !appliedTxs.exists(t => t.id sameElements tx.id) && {
        state match {
          case v: TransactionValidator => v.validate(tx).isSuccess
          case _ => true
        }
      }
    }
  ...
  protected def handleSubscribe: Receive = {
    case EncryBaseNodeViewHolder.Subscribe(events) =>
      events.foreach { evt =>
...
object EncryBaseNodeViewHolder {
  ...
  case class ChangedMempool[MR <: EncryMempoolReader](mempool: MR) extends NodeViewChange
  ...
  case class FailedTransaction[TX <: EncryBaseTransaction](transaction: TX, error: Throwable) extends ModificationOutcome

  case class SuccessfulTransaction[TX <: EncryBaseTransaction](transaction: TX) extends ModificationOutcome
