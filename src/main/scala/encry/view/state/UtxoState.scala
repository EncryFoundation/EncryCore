...
class UtxoState(override val persistentProver: PersistentBatchAVLProver[Digest32, HF],
  ...
  override def applyModifier(mod: EncryPersistentModifier): Try[UtxoState] = mod match {
    ...
    case block: EncryBlock =>
      ...
      val rootBeforeApplication: ADDigest = rootHash
      ...
      applyBlockTransactions(block.payload.transactions, block.header.stateRoot).map { _ =>
        ...
        logWarn(s"Failed to apply block with header ${block.header.encodedId} to UTXOState with root" +
          s" ${Algos.encode(rootBeforeApplication)}: ", e)
        Failure(e)
      }
    ...
    stateStore.get(ByteArrayWrapper(version)) match {
      case Some(v) =>
        val rollbackResult: Try[UtxoState] = prover.rollback(ADDigest @@ v.data).map { _ =>
          ...
          new UtxoState(prover, version, Height @@ stateHeight, stateStore, lastBlockTimestamp, nodeViewHolderRef)
        }
