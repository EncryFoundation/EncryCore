...
case class EncryStateView(height: Height, lastBlockTimestamp: Long) extends PConvertible {
  ...
  override def asVal: PValue = PValue(PObject(Map(
    ...
    "stateDigest" -> PValue(ADDigest @@ Array.emptyByteArray, Types.PCollection.ofByte)
  ), tpe), tpe)
}