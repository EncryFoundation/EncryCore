...
class EncryMiningWorker(myIdx: Int, numberOfWorkers: Int) extends Actor with Logging {
  ...
  def miningInProgress: Receive = {
    case MineBlock(candidate: CandidateBlock, nonce: Long) =>
      log.info(s"Trying nonce: $nonce. Start nonce is: ${Long.MaxValue / numberOfWorkers * myIdx}. " +
      ...
      ConsensusSchemeReaders.consensusScheme.verifyCandidate(candidate, nonce)
        .fold(self ! MineBlock(candidate, nonce + 1)) { block =>
          log.info(s"New block is found: $block on worker $self at ${sdf.format(new Date(System.currentTimeMillis()))}. Iter qty: ${nonce - (Long.MaxValue / numberOfWorkers * myIdx) + 1}")
  ...
  def miningPaused: Receive = {
    case NextChallenge(candidate: CandidateBlock) =>
      ...
      log.info(s"Start challenge on worker: $myIdx at height ${candidate.parentOpt.map(_.height + 1).getOrElse("No parent")} at ${sdf.format(challengeStartTime)}")
