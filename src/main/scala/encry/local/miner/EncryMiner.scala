...
class EncryMiner extends Actor with ScorexLogging {
  ...
  def shouldStartMine(b: EncryBlock): Boolean = encrySettings.node.mining && b.header.timestamp >= startTime
    ...
    case StartMining =>
      candidateOpt match {
        case Some(candidateBlock) =>
          isMining = true
          val numberOfWorkers: Int = encrySettings.node.numberOfMiningWorkers
  ...
  def produceCandidate(): Unit =
    nodeViewHolder ! GetDataFromCurrentView[EncryHistory, UtxoState, EncryWallet, EncryMempool, CandidateEnvelope] { view =>
      ...
      if (bestHeaderOpt.isDefined || encrySettings.node.offlineGeneration)
