...
class StatsSender(influxDBSettings: InfluxDBSettings, networkSettings: NetworkSettings, constants: Constants)
    extends Actor
    with StrictLogging {
  ...
  override def receive: Receive = {
    ...
    case GetModifiers(_, modifiers) =>
      modifiers
        .foreach(
          downloadedModifierId =>
    ...
    case MiningEnd(blockHeader, workerIdx, workersQty) =>
      timeProvider
        .time()
        .map(
          time =>
    ...
    case EndOfApplyingModifier(modifierId) =>
      modifiersToApply.get(Algos.encode(modifierId)).foreach { modInfo =>
        influxDB.write(influxDBSettings.udpPort,
                       s"modifApplying,nodeName=$nodeName," +
    ...
    case StateUpdating(time) =>
      influxDB.write(influxDBSettings.udpPort, s"stateUpdatingTime,nodeName=$nodeName value=$time")
    ...
    case UtxoStat(txsNumber: Int, validationTime: Long) =>
      influxDB.write(influxDBSettings.udpPort,
                     s"utxoStat,nodeName=$nodeName txsNumber=$txsNumber,value=$validationTime")

    case msg: ModifiersDownloadStatistic =>
      msg match {
    ...
    case SendDownloadRequest(modifierTypeId: ModifierTypeId, modifiers: Seq[ModifierId]) =>
      modifiersToDownload = modifiersToDownload ++ modifiers.map(
        mod => (Algos.encode(mod), (modifierTypeId, System.currentTimeMillis()))
      )
  }
}
...
object StatsSender {
  ...
  final case class ValidatedModifierFromNetwork(modifierTypeId: ModifierTypeId)
      extends ModifiersDownloadStatistic
      with StatsSenderMessage
